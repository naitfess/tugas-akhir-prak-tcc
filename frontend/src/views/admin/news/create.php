...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch(`${beUrl}/api/categories` )
        ...
        .catch(() => {
            // Jika gagal, tampilkan satu option error
            const select = document.getElementById('category-select');
    ...
    if (form) {
        form.addEventListener('submit', function (e) {
            ...
            if (!title || !content || !date || imageFile === null || categoryIds.length === 0) {
                alert('Title, content, image, category and date are required.');
                return;
            }
            ...
            .then(({ status, body }) => {
                if (status === 201) {
                    ...
                    alert(body.message || 'Gagal menambah news');
                }
            })
            .catch(error => {
                alert('Terjadi kesalahan saat menambah news');
                console.error(error);
            })
            .finally(() => {
